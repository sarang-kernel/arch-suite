...
/// Creates a system snapshot by gathering configs, package lists, and dotfiles.
pub fn action_create_snapshot() -> AppAction {
    Box::pin(async {
        // Use `whoami` to get the non-sudo username, which is more reliable.
        ...
        // This command must be run with `sudo sh -c "..."` because some sub-commands need root.
        let command_script = format!(
            "echo 'Gathering package lists...'; \
             pacman -Qqe > {0}/packages.x86_64.txt && \
             pacman -Qqm > {0}/packages.foreign.txt && \
             echo 'Archiving /etc...'; \
             tar -czf {0}/etc.tar.gz /etc && \
             echo 'Archiving dotfiles for user {3}...'; \
        ...
        // We must use `sudo sh -c` to correctly handle permissions for all sub-commands.
...
pub async fn check_and_install_dependencies() -> Result<bool> {
    ...
    for dep in &deps {
        // Use tokio's Command for async execution.
    ...
    if missing_deps.is_empty() {
        ...
        // A small delay so the user can see the message before the TUI starts.
